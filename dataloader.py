...
'''
    ...
    observed_data: shape (win_size, feature_size) 一个样本的观测数据
    observed_mask: shape (win_size, feature_size) 和observed_data的shape相同，全为1
    gt_mask: shape (win_size, feature_size) 和observed_data的shape相同，全为1
    timepoints: shape (win_size,) 时间点的索引，从0到win_size-1  
    feature_id: shape (feature_size,) 特征的索引，从0到feature_size-1
    label: shape (win_size,) 标签数据，
    其中每个元素表示对应时间点的标签，通常为0或1，
    0表示正常，1表示异常。
...
root_path = '/home/xuke/test_code/TSDE/data/anomaly_detection/'
...
class PSMSegLoader(Dataset):
    ...
    - step: The step size for sliding window.   # 滑动窗口的步长  默认100 相当于没有重复
    - flag: Indicates the dataset split to use ('train', 'val', 'test').

    Attributes:
    - train: Training data after scaling.   # 缩放后的训练数据
    ...
    def __init__(self, win_size, step=100, flag="train"):
        ...
        data = data.values[:, 1:]  # DataFrame转换为numpy数组, 去掉第一列
        data = np.nan_to_num(data) # 将nan替换为0
        print(flag, "data shape:", data.shape)
        self.scaler.fit(data)
        data = self.scaler.transform(data)  # 缩放了数据
        test_data = pd.read_csv(os.path.join(self.root_path, 'test.csv'))
        test_data = test_data.values[:, 1:]
        test_data = np.nan_to_num(test_data)
        self.test = self.scaler.transform(test_data)
        self.train = data
        data_len = len(self.train)  # 训练数据的长度
        self.val = self.train[(int)(data_len * 0.8):]  # train data的20%作为验证数据集
    ...
    def __len__(self):
        if self.flag == "train":
            return (self.train.shape[0] - self.win_size) // self.step + 1   # 相当于最后的长度不满足win_size的部分不用了
        elif (self.flag == 'val'):
...
def anomaly_detection_dataloader(dataset_name, win_size=100, batch_size=32):
    """
...
if __name__ == '__main__':
    ...
    patch = data[2]
    print("observed_data:", patch['observed_data'].shape)
    print("observed_mask:", patch['observed_mask'].shape)
    print("gt_mask:", patch['gt_mask'].shape)
    print("timepoints:", patch['timepoints'].shape)
    print("feature_id:", patch['feature_id'].shape)
    print("label:", patch['label'].shape)
    print("observed_data:", patch['observed_data'])
    print("observed_mask:", patch['observed_mask'])
    print("gt_mask:", patch['gt_mask'])
    print("timepoints:", patch['timepoints'])
    print("feature_id:", patch['feature_id'])
    print("label:", patch['label'])
